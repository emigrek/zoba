...
const Input: FC<InputProps> = forwardRef<HTMLInputElement, InputProps>(({ right, className, size, ...props }, ref) => {
    return (
        <div className={cn(
            "flex h-12 w-full rounded-full transition duration-200 ease-in-out border border-neutral-700 bg-transparent py-3 px-4 text-sm focus-within:ring-2 focus-within:ring-neutral-700",
            className
        )}>
            <input
